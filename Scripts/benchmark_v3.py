...
def read_bmrb(fname=None, ename=None):
  ...
  for item_ls in new_atom_chem_shift_data:
    line_ls_a = [item_ls[4], item_ls[6], item_ls[9]]
...
def create_noesy_peak_list(pdb_list,noesy_type,cs_list,distMat):
  '''
  rewriting function without using keylist for sake of calculating distance for intermolecular domain
  ...
  for i in range(len(pdb_list)):
    nSeq, x, y, z, aaa, a, atm, c = pdb_list[i]
    # changing from 32 to 12 nomenclature for atoms
    atm = proton_nomenclature.s32tos12(a,atm)
      # get heavy atoms: N or C
    try:  
    ...
    try:
        nc_shift = get_shift(str(nSeq), ncatm, cs_list)
        # print (nc_shift, ncatm)
    except:
      continue

    try:
        # get proton atom shift
        h_shift = get_shift(str(nSeq), atm, cs_list)
    except:
      continue
    ...
    for j in range(i+1, len(pdb_list)):
      nSeq2, x2, y2, z2, aaa2, a2, atm2, c2 = pdb_list[j]
      dist = distMat[int(nSeq), int(nSeq2), c_list.index(c), c_list.index(c2)] = \
              distMat[int(nSeq2),int(nSeq),c_list.index(c2),c_list.index(c)]
            
      print (dist)
      # avoid sequential data; only want intermolecular domain
      if (nSeq, x, y, z, aaa, a, atm, c) == (nSeq2, x2, y2, z2, aaa2, a2, atm2, c2):
         continue
      if c == c2: # A - B chains only
         continue
      if dist == 0:
         continue
      if dist > 5.5:
      ...
      # get thru space proton shifts 
      try:
        h2_shift = get_shift(str(nSeq2), atm2, cs_list)
      except:
        continue
      if h2_shift < -1000:
        continue
...
# [HA-H],[H], frq, proton_dist_probability, normailzed frq, Bayes' Theorem, normalized Bayes', endurance score
  ...
  for k in combination:
    ...
    # get chains ID
    try:
      c = aa_dict[catm, int(cnSeq)][-1]
    except:
       continue
    try:
      c2 = aa_dict[catm2, int(cnSeq2)][-1]
    except:
       continue
    ...
    if endurance_score == 0:
       continue
    try:
      x,y,z = aa_dict[catm, int(cnSeq)][0:3]
    except:
      continue
    try:  
      x2,y2,z2 = aa_dict[catm2,int(cnSeq2)][0:3]
    except:
      continue
...
for PDB in os.listdir(AF_pdb_filepath):
  ...
  if AF_path.endswith('H.pdb'):
    ...
    AF_line = create_noesy_peak_list(AF_readPDB, NoesyType, cs_list,AF_distMat)
    i += 1
    # write out NOESY peak list for each AF structure
    AF_PDB_text = open (f'{work_directory}/Dimer/{NoesyType[0].upper()}_NOESY_{PDB_id}_{i}.list', 'w')
    AF_PDB_text.write(AF_line)
    AF_PDB_text.close()
# chosen PDB for analysis
pdbList = f'{AF_pdb_filepath}/fold_2n74_deepmind_model_0_H.pdb'
# print (readPDB(pdbList,1))
Noesy_file_path = f'{work_directory}/Dimer/{NoesyType[0].upper()}_NOESY_{PDB_id}_1.list'
