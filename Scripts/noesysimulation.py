...
This script meticulously compares NOESY data from two sources: the best-fitting model generated by AlphaFold-Multimer and the PDB file of a selected monomer/protein. It will show the number of similarities and differences in peaks between w1, w2, and w3. The different or uncommon peaks represent potential interface signals of the homodimer protein, whereas the common peaks represent intra-interactions of the monomer.
"""
...
def create_noesy_peak_list(keyList,noesy_type='cnoe'):
...
Alt_pdb_file = '/content/interfacePDB'
# open interface zip file and iterate over PDBs
if not os.path.exists(Alt_pdb_file):
  !unzip /content/interfacePDB.zip -d /content/interfacePDB
  if os.path.endswith('.pdb'):
