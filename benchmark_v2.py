...
def aa_contact_map(pdbList):
    ...
    renumbered_pdbList = renumber_seq_PDB(pdbList)
    readPDB_list = readPDB(renumbered_pdbList, 1, nuclei=['C',])
...
def distance_probability (pdbList):
  renumbered_pdbList = renumber_seq_PDB(pdbList)
  filtered_pdb_list = readPDB(renumbered_pdbList,1,nuclei=['H',])
...
def aa_dictionary(pdbList):
  renumbered_pdbList = renumber_seq_PDB(pdbList)
  readPDB_list = readPDB(renumbered_pdbList, 1, nuclei = ['H',])
  aa_dict = {}
  for i in range(len(readPDB_list)):
      nSeq, x, y, z, aaa, a, atm, c = readPDB_list[i]
...
# open AF PDB of protein with the protons added
  for PDB in os.listdir(AF_pdb_filepath):
    ...
    if AF_path.endswith('H.pdb'):
      AF_renumbered_list = renumber_seq_PDB(AF_path)
      AF_readPDB = readPDB(AF_renumbered_list,1,nuclei = ['H',])
  ...
  f = open(f'{work_directory}/{POKY_job_id}/BestEvaluated/all_alt.tbl', 'w')
  f.write(new_content)
  f.close()  
